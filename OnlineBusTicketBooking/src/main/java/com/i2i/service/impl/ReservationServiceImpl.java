...
public class ReservationServiceImpl extends GenericManagerImpl<Reservation, Long> implements ReservationService {
     ...
     */
    public Reservation addReservation (User user, TripRoute tripRoute, int noOfSeatsBooked, double totalPrice, String paymentMode, boolean status) throws DatabaseException, InputException {
    	if (InputValidationUtil.checkIfPaymentModeIsValid(paymentMode)) {
    		if (InputValidationUtil.checkIfNotZero(noOfSeatsBooked)) {
        		...
        		throw new InputException ("Please Enter a valid number of seats");
        	}
    	} else {
    		System.out.println("NO PAYMENT MODE");
    		throw new InputException ("Please Enter a valid Payment Mode");
    	}
    }
